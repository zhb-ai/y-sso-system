  ...
  <div class="page-container">
      ...
      >
        ...
        <el-table-column label="角色" min-width="150">
          <template #default="scope">
            ...
            >{{ role.name }}</el-tag>
            <el-text v-if="!(scope.row.roles || []).length" type="info" size="small">无角色</el-text>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="380" align="center" fixed="right" class-name="table-cell-flex-center">
          <template #default="scope">
            <el-button type="primary" size="small" link @click="handleEdit(scope.row)">
              <el-icon><Edit /></el-icon> 编辑
            </el-button>
            <el-button type="success" size="small" link @click="handleAssignRole(scope.row)">
              <el-icon><Medal /></el-icon> 角色
            </el-button>
            <el-button type="warning" size="small" link @click="handleAssignSSORole(scope.row)">
              <el-icon><Connection /></el-icon> SSO
            </el-button>
            <el-button type="warning" size="small" link @click="handleToggleStatus(scope.row)">
              <el-icon><component :is="scope.row.status === 'active' ? Lock : Unlock" /></el-icon> {{ scope.row.status === 'active' ? '禁用' : '启用' }}
            </el-button>
            <el-button type="primary" size="small" link @click="handleResetPassword(scope.row)">
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="roleDialogSaving" @click="handleSaveRoles">保存</el-button>
      </template>
    </el-dialog>
    ...
    >
        ...
        <div class="section-block">
          ...
          <div class="section-block__content" v-loading="ssoRoleDialogLoading">
            ...
            <div v-if="allSSORoles.length === 0" class="section-block__empty">
              <el-empty description="暂无 SSO 角色，请先在「SSO 角色管理」中创建" :image-size="60" />
            </div>
          </div>
        </div>
      </div>
      <template #footer>
        ...
        <el-button type="primary" :loading="ssoRoleDialogSaving" @click="handleSaveSSORoles">保存</el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button type="primary" @click="handleResetPasswordSubmit" :loading="resetPasswordLoading">确定重置</el-button>
        </div>
      </template>
    </el-dialog>
    ...
    >
      ...
      <template #footer>
        <el-button type="primary" @click="handleCopyPassword">复制密码</el-button>
        <el-button @click="resetPasswordResultVisible = false">知道了</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const handleSaveRoles = async () => {
  const toAdd = selectedRoleCodes.value.filter(c => !originalRoleCodes.value.includes(c))
  const toRemove = originalRoleCodes.value.filter(c => !selectedRoleCodes.value.includes(c))
