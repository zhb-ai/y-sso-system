  ...
  <div class="page-container">
      ...
      <div class="header-actions">
        ...
        <el-button type="danger" @click="handleClearAll" :loading="clearingAll">
          ...
          <el-tooltip
            ...
            placement="top"
          >
            <el-icon class="hint-icon btn-hint-icon"><QuestionFilled /></el-icon>
          </el-tooltip>
        </el-button>
      </div>
    </div>
    ...
    <el-row :gutter="16" class="summary-row">
      <el-col :xs="24" :sm="12" :md="6">
        <el-card class="summary-card" shadow="hover">
          ...
          <div class="summary-value">{{ summary[CACHE_PAGE_KEYS.SUMMARY_TOTAL_FUNCTIONS] }}</div>
        </el-card>
      </el-col>
      <el-col :xs="24" :sm="12" :md="6">
        <el-card class="summary-card" shadow="hover">
          <div class="summary-title">总命中次数</div>
          <div class="summary-value">{{ summary[CACHE_PAGE_KEYS.SUMMARY_TOTAL_HITS] }}</div>
        </el-card>
      </el-col>
      <el-col :xs="24" :sm="12" :md="6">
        <el-card class="summary-card" shadow="hover">
          <div class="summary-title">总未命中次数</div>
          <div class="summary-value">{{ summary[CACHE_PAGE_KEYS.SUMMARY_TOTAL_MISSES] }}</div>
        </el-card>
      </el-col>
      <el-col :xs="24" :sm="12" :md="6">
        <el-card class="summary-card" shadow="hover">
          <div class="summary-title">总命中率</div>
          <div class="summary-value">{{ formatSummaryHitRate(summary[CACHE_PAGE_KEYS.SUMMARY_TOTAL_HIT_RATE]) }}</div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-card class="filter-card" shadow="hover">
      <el-form :inline="true" class="filter-form">
        ...
        <el-form-item>
          <el-button type="primary" @click="handleViewSingleStats()" :disabled="!selectedFunctionName">
            查看单函数统计
          </el-button>
    ...
    <el-card class="data-card" shadow="hover">
      <el-table v-loading="loading" :data="functions" row-key="name" style="width: 100%">
        <el-table-column :prop="CACHE_FUNCTION_COLUMNS.NAME" label="函数名" min-width="180" />
        <el-table-column :prop="CACHE_FUNCTION_COLUMNS.MODULE" label="模块" min-width="260" show-overflow-tooltip />
        <el-table-column :prop="CACHE_FUNCTION_COLUMNS.BACKEND" label="后端" width="120" align="center">
          <template #default="scope">
            <el-tag :type="getBackendTagType(scope.row[CACHE_FUNCTION_COLUMNS.BACKEND])" effect="light">
              {{ scope.row[CACHE_FUNCTION_COLUMNS.BACKEND] }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column :prop="CACHE_FUNCTION_COLUMNS.TTL" label="TTL(秒)" width="120" align="center" />
        <el-table-column :prop="CACHE_FUNCTION_COLUMNS.KEY_PREFIX" label="Key 前缀" min-width="180" show-overflow-tooltip />
          ...
          <template #header>
            <span>
              ...
              <el-tooltip
                ...
                placement="top"
              >
                <el-icon class="hint-icon table-header-hint"><QuestionFilled /></el-icon>
              </el-tooltip>
            </span>
          </template>
          <template #default="scope">
        ...
        <el-table-column label="操作" width="180" align="center" class-name="table-cell-flex-center">
          <template #default="scope">
            <el-button type="info" link size="small" @click="handleViewEntries(scope.row.name)">
              条目
            </el-button>
            <el-button type="primary" link size="small" @click="handleViewSingleStats(scope.row.name)">
              统计
            </el-button>
            <el-button type="danger" link size="small" @click="handleClearOne(scope.row.name)">
              清空
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="singleStatsDialogVisible" title="缓存函数统计" width="520px">
      <el-descriptions :column="1" border v-if="singleStats">
        <el-descriptions-item label="函数">{{ singleStats.function || '-' }}</el-descriptions-item>
        <el-descriptions-item label="后端">{{ singleStats.backend || '-' }}</el-descriptions-item>
        <el-descriptions-item label="TTL(秒)">{{ singleStats.ttl ?? '-' }}</el-descriptions-item>
        <el-descriptions-item label="命中次数">{{ singleStats.hits ?? 0 }}</el-descriptions-item>
        <el-descriptions-item label="未命中次数">{{ singleStats.misses ?? 0 }}</el-descriptions-item>
        <el-descriptions-item label="命中率">{{ singleStats.hit_rate || '-' }}</el-descriptions-item>
      </el-descriptions>
      <template #footer>
    ...
    >
      ...
      <div class="data-card dialog-table-card">
        <el-table v-loading="entriesLoading" :data="entriesList" row-key="key" style="width: 100%">
          <el-table-column prop="key" label="Key" min-width="280" show-overflow-tooltip />
          <el-table-column prop="ttl_remaining" label="剩余TTL(秒)" width="140" align="center">
            <template #default="scope">
              {{ scope.row.ttl_remaining ?? '-' }}
            </template>
          </el-table-column>
          <el-table-column prop="value_type" label="值类型" width="120" align="center" />
          <el-table-column prop="value_size" label="大小" width="120" align="center">
            <template #default="scope">
              {{ formatSize(scope.row.value_size) }}
            </template>
          </el-table-column>
          <el-table-column label="操作" width="100" align="center" class-name="table-cell-flex-center">
            <template #default="scope">
              <el-button type="primary" link size="small" @click="handleViewEntryDetail(scope.row.key)">
                详情
              </el-button>
            </template>
          </el-table-column>
      ...
      <div class="entry-detail-block" v-loading="entryDetailLoading">
        <div class="entry-detail-header">
          ...
          <el-tooltip
            ...
            placement="top"
          >
            <el-icon class="hint-icon entry-detail-hint"><QuestionFilled /></el-icon>
          </el-tooltip>
        </div>
        <el-empty
          v-if="!entryDetail"
        ...
        <template v-else>
          <el-descriptions :column="2" border class="entry-detail-meta">
            <el-descriptions-item label="Key">{{ entryDetail.key }}</el-descriptions-item>
            <el-descriptions-item label="值类型">{{ entryDetail.value_type }}</el-descriptions-item>
            <el-descriptions-item label="剩余TTL">{{ entryDetail.ttl_remaining ?? '-' }}</el-descriptions-item>
            <el-descriptions-item label="大小">{{ formatSize(entryDetail.value_size) }}</el-descriptions-item>
          </el-descriptions>
          <div class="preview-title">值预览</div>
          <pre class="preview-json">{{ formatPreview(entryDetail.value_preview) }}</pre>
        </template>
      </div>
...
const handleViewSingleStats = async (functionName = selectedFunctionName.value) => {
