              ...
              <el-tabs v-model="editManageTab" stretch>
                ...
                <el-tab-pane name="org">
                  ...
                  <div class="section-block__table">
                    <el-table
                      :data="currentEmployee?.organizations || []"
                      size="small"
                      max-height="260"
                    >
                      <el-table-column
            ...
            >
              <el-form-item label="回调 Token">
                <el-input
                  ...
                  placeholder="事件服务器 Token"
                  autocomplete="off"
                />
              </el-form-item>
              <el-form-item label="EncodingAESKey">
                <el-input
                  v-model="wechatForm.callback_aes_key"
                  placeholder="事件服务器 EncodingAESKey"
                  autocomplete="off"
                />
              </el-form-item>
              <el-form-item v-if="wechatBindMode !== 'view'" label="回调 URL">
                <el-input :model-value="wechatWebhookUrl" readonly disabled>
                  <template #append>
...
const getPrimaryOrgRel = (employee = {}) => {
  ...
  return (
    orgs.find((org) => org.org_id === currentOrgId.value) ||
    orgs.find((org) => org.is_primary) ||
    orgs[0] ||
    null
  );
};
...
const handleDeleteDept = async (dept) => {
  try {
    await ElMessageBox.confirm(`确定删除部门「${dept.name}」吗？`, "确认删除", {
      type: "warning",
    });
  ...
  try {
    if (employeeForm.id) {
      ...
      if (newEmpId && employeeForm.create_account) {
        try {
          const accountRes = await employeeApi.createAccount({
            employee_id: newEmpId,
          });
...
const handleAddExistingToDept = async (emp) => {
  try {
    ...
    const isInOrg = empDetail.data?.organizations?.some(
      (o) => o.org_id === currentOrgId.value,
    );
...
.empty-org-card :deep(.el-card__body) {
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: center;
